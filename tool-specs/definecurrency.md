...
| `preallocations` | object[] | No | Mint tokens to specific VerusIDs at launch. Array of `{"name@": amount}` (friendly name) or `{"i-address": amount}`. R-addresses are not accepted. Works with both simple tokens and baskets. For baskets, this lowers the reserve ratio (new supply minted without adding reserves). |
